...
public class TopicsAdapter extends ListAdapter<Topic, TopicsAdapter.TopicsViewHolder> {
    ...
    protected TopicsAdapter() { // need to
        super(new DiffUtil.ItemCallback<Topic>() { // need to define that for ListAdapter knew how to correctly work with out List.
            // And we won't need to create any instance variables to work with List.
            @Override
    ...
    public TopicsViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View layout = LayoutInflater.from(parent.getContext()).inflate(android.R.layout.activity_list_item, parent, false);
        return new TopicsViewHolder(layout);
    }

    @Override
    public void onBindViewHolder(@NonNull TopicsViewHolder holder, int position) { // ListAdapter contains some methods which will allow us reduce the number of methods. For example - in finding positions on View Holder.
        holder.bind(getItem(position), listener);
    }

    public static class TopicsViewHolder extends RecyclerView.ViewHolder {  // hide ref on its super class
